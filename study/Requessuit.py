...
def create_login_window(root_ref):
    # Your code to create the login window goes here
    import hashlib
    def handle_login():
    ...
    # After creating the login window, start the main loop again
    root_ref.mainloop()
#---------------------------------------------------------------------
# function for subdomain
def subdomain(root_ref):
    ...
    welcome_label = Label(dashboard_frame, text='SubSuit Dashboard!', font=('Arial', 20), padx=20, pady=20)
    welcome_label.pack()
#-----------------------------------------------------------------------------------------------------------------------
    # add domain title of text box



    domains_label = Label(root, text='Enter the Domain', fg='white', bg='#0096DC')
    domains_label.place(x=330, y=100)
    domains_label.config(font=('verdana', 20))

    # add domain box
    domains_input = Text(root, width=30, height=1.4, font=('Arial', 15))
    domains_input.place(x=285, y=150)

    # run scan button & function
    sub_btn = Button(dashboard_frame, text='Start Brute', bg='white', fg='black', width=10, height=1, command= lambda: run_subdomain(root_ref))  #
    sub_btn.config(font=('verdana', 15))
    sub_btn.place(x=380, y=200)

    # goback button and function
    goback_button = Button(dashboard_frame, text='GoBack!', bg='white', fg='black', width=15, height=1,command=lambda: main_dashboard(root_ref))
    goback_button.config(font=('verdana', 15))
    goback_button.place(x=85, y=10)

    # logout buttton and function
    logout2_button = Button(dashboard_frame, text='Logout', bg='white', fg='black', width=15, height=1,command=lambda: handle_logout(root_ref))
    logout2_button.config(font=('verdana', 15))
    logout2_button.place(x=615, y=10)

    # -----------------------------------------------------------------------------------------------------------------------
    # output_label = Label(root, text='Output', fg='white', bg='#0096DC')
    # output_label.place(x=330, y=250)
    # output_label.config(font=('verdana', 20))

    result_label = Label(dashboard_frame, text='Output Area', font=('Arial', 14), padx=10,pady=10)  # bg='#0096DC',fg='white'
    result_label.place(x=390, y=260)

    # add domain box
    output_input = Text(root, width=50, height=15, font=('Arial', 15))
    output_input.place(x=170, y=320)


    #-----------------------------------------------------------------------------------------------------------------------
    def run_subdomain(root_ref):
        import requests
        from concurrent.futures import ThreadPoolExecutor

        dom = domains_input.get("1.0", "end-1c")

        # Function to check validity of subdomains
        def check_subdomain(subdomain, domain_name):
            url = f"https://{subdomain}.{domain_name}"
            try:
                response = requests.head(url)
                if response.status_code < 400:
                    output_input.insert(END, 'Success!\n')
                    return url
            except requests.ConnectionError:
                output_input.insert(END, f'Other error occurred - {err}\n')
                pass
            return None
        ...
        def domain_scanner(domain_name, sub_domnames):
            ...
            def verbose_progress(subdomain):
                url = f"https://{subdomain}.{domain_name}"
                print(f"Checking: {url}")

            with ThreadPoolExecutor(max_workers=20) as executor:  # Adjust the number of workers as needed
                futures = [executor.submit(check_subdomain, subdomain, domain_name) for subdomain in sub_domnames]

                for future in futures:
                    result = future.result()
                    if result:
                        valid_urls.append(result)
                        verbose_progress(sub_domnames[len(valid_urls) - 1])
        ...
        if __name__ == '__main__':
            ...
            dom_name = dom  # input("Enter the Domain Name:")
            # print('\n')
            ...
            # writing the valid urls to a file in append mode
            # writing the valid urls to a file
...
def handle_logout(root_ref):
    # Destroy the current dashboard or logged-in session
    # Then open the login window again
    for widget in root_ref.winfo_children():
        widget.destroy()

    create_login_window(root_ref)
    #root = Tk()
    root_ref.mainloop()
...
def main_dashboard(root_ref):
    ...
    # Add widgets and functionality for the dashboard
    dashboard_label = Label(dashboard_frame, text='Main Dashboard!', font=('Arial', 20), padx=50, pady=20)
    dashboard_label.pack()


    # domain button & function
    request_button = Button(dashboard_frame, text='Requestsuit', bg='white', fg='black', width=15, height=1,command=lambda: requestsuit_dashboard(root_ref)) #
    request_button.config(font=('verdana', 15))
    request_button.place(x=240, y=100)

    # domain button & function
    subdomain_button = Button(dashboard_frame, text='SubSuit', bg='white', fg='black', width=15, height=1,command=lambda: subdomain(root_ref)) #
    subdomain_button.config(font=('verdana', 15))
    subdomain_button.place(x=450, y=100)

    # logout button & function
    logout_button = Button(dashboard_frame, text='Logout', bg='white', fg='black', width=10, height=1,command=lambda: handle_logout(root_ref))  #
    logout_button.config(font=('verdana', 15))
    logout_button.place(x=375, y=150)
...
def requestsuit_dashboard(root_ref):
    ...
    # -----------------------------------------------------------------------------------------------------------------------
    # goback button and function
    goback_button = tk.Button(dashboard_frame, text='GoBack!', bg='white', fg='black', width=15, height=1,command=lambda: main_dashboard(root_ref))
    goback_button.config(font=('verdana', 15))
    goback_button.place(x=60, y=10)

    # logout buttton and function
    logout2_button = tk.Button(dashboard_frame, text='Logout', bg='white', fg='black', width=15, height=1,command=lambda: handle_logout(root_ref))
    logout2_button.config(font=('verdana', 15))
    logout2_button.place(x=635, y=10)

    # domain button & function
    domain_btn = Button(dashboard_frame, text='Start Here', bg='white', fg='black', width=10, height=1) #
