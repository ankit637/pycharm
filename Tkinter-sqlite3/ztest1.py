...
def open_dashboard(root_ref):
    ...
    #
    #     # Reopen the root window or create a new one
    #     root.deiconify()  # Show the hidden root window or recreate it if needed

    def logout():
        # Destroy all the widgets in the dashboard_frame
        for widget in dashboard_frame.winfo_children():
            widget.destroy()


        # Destroy the dashboard frame itself
        dashboard_frame.destroy()
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    subd_btn = Button(dashboard_frame, text='SubSuit', bg='white', fg='black', width=15, height=1, command=html)
    subd_btn.config(font=('verdana', 15))
    # subd_btn.pack(pady=(10, 20), anchor='nw')
    subd_btn.place(x=10, y=10)

    logout_btn = Button(dashboard_frame, text='Logout', bg='white', fg='black', width=15, height=1, command=logout)
    logout_btn.config(font=('verdana', 15))
    # subd_btn.pack(pady=(10, 20), anchor='nw')
    logout_btn.place(x=10, y=60)
